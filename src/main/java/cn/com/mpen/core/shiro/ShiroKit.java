...
public class ShiroKit {
    ...
    public static boolean isGeneralAdmin() {
        ...
        for (Integer integer : roleList) {
            String singleRoleTip = ConstantFactory.me().getSingleRoleTip(integer);
            if (singleRoleTip.equals(Const.GENERALADMIN_NAME)) {
                return true;
            }
        }
        return false;
     ...
     * 判断当前用户是否是出版社管理员
     */
    public static boolean isPublishAdmin() {
        List<Integer> roleList = ShiroKit.getUser().getRoleList();
        for (Integer integer : roleList) {
            String singleRoleTip = ConstantFactory.me().getSingleRoleTip(integer);
            if (singleRoleTip.equals(Const.PUBLISH_NAME)) {
                return true;
            }
        }
        return false;
    ...
    public static List getUserids() {
        ...
        userIds.add(ShiroKit.getUser().id);
        if(roleList.size()>0){
            int roleId = roleList.get(0);
            if(roleId == Const.PUBLISH_ROLE_ID){
                //说明是出版社 （查询出版社管理员下面所有的编辑）
                EntityWrapper<User> wrapper = new EntityWrapper<>();
                wrapper.eq("userid", ShiroKit.getUser().id);
                List<Map<String, Object>> users = userService.selectMaps(wrapper);
                for (Map<String, Object> user : users) {
                    userIds.add((Integer) user.get("id"));
                }
            }else if (roleId == Const.EDIT_ROLE_ID) {
                //说明是编辑（通过自身ID查询上级出版社ID，再查询出版社管理员下面所有的编辑）
                EntityWrapper<User> wrapper = new EntityWrapper<>();
                wrapper.eq("id", ShiroKit.getUser().userid);
                List<Map<String, Object>> users = userService.selectMaps(wrapper);
                for (Map<String, Object> user : users) {
                    userIds.add((Integer) user.get("id"));
                }
    ...
    //管理员查询出版社统计获取出版社所有id
    public static List getUserids(Integer userId) {
        ...
        if (userId !=null) {
            ...
            List<Map<String, Object>> users = userService.selectMaps(wrapper);
            userIdList.add(userId); //封装自己的id
            for (Map<String, Object> user : users) { //封装下级id
